...
export const lsp_symbols: ToolDefinition = tool({
  description: "获取文件符号（scope='document'）或在工作区中搜索符号（scope='workspace'）。",
  args: {
    filePath: tool.schema.string().describe("用于 LSP 上下文的文件路径"),
    scope: tool.schema
      .enum(["document", "workspace"])
      .default("document")
      .describe("'document' 获取文件符号，'workspace' 在工作区搜索"),
    query: tool.schema.string().optional().describe("符号名称（workspace 必填）"),
  ...
  execute: async (args, context) => {
    try {
      ...
      if (scope === "workspace") {
        if (!args.query) {
          return "错误：workspace 作用域必须提供 query"
        }
      ...
      const lines: string[] = []
      if (truncated) {
        lines.push(`共找到 ${total} 个符号（显示前 ${limit} 个）：`)
      }
