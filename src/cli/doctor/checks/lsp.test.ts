...
describe("lsp check", () => {
  ...
  describe("checkLspServers", () => {
    ...
    it("returns warn when no servers installed", async () => {
      ...
      expect(result.message).toMatch(/No LSP servers|未检测到\s*LSP\s*服务器/)
    })
    ...
    it("lists installed and missing servers in details", async () => {
      ...
      expect(result.details?.some((d) => d.includes("Installed") || d.includes("已安装"))).toBe(true)
      expect(result.details?.some((d) => d.includes("Not found") || d.includes("未找到") || d.includes("缺少"))).toBe(true)
    })
  })
