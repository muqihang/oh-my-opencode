...
describe("prometheus-md-only", () => {
  const TEST_SESSION_ID = `test-session-prometheus-${randomUUID()}`
  ...
  beforeEach(() => {
    testDirectory = join(tmpdir(), `prometheus-md-only-dir-${randomUUID()}`)
  })
  ...
  afterEach(() => {
   ...
   describe("with Prometheus agent in message storage", () => {
    ...
    test("should allow Prometheus to write .md files inside .sisyphus/", async () => {
      ...
      const output = {
        args: { filePath: `${testDirectory}/.sisyphus/plans/work-plan.md` },
      }
    ...
    test("should NOT inject workflow reminder for .sisyphus/drafts/", async () => {
      ...
      const output: { args: Record<string, unknown>; message?: string } = {
        args: { filePath: `${testDirectory}/.sisyphus/drafts/notes.md` },
      }
