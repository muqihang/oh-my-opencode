...
describe("executeCompact lock management", () => {
    ...
    expect(mockClient.tui.showToast).toHaveBeenCalledWith(
      expect.objectContaining({
        body: expect.objectContaining({
          title: expect.stringMatching(/正在压缩|Compact In Progress/),
          message: expect.stringMatching(/恢复已在进行|Recovery already running/),
          variant: "warning",
        }),
      }),
    )
  ...
  test("prevents concurrent compaction attempts", async () => {
    ...
    const blockedToast = toastCalls.find(
      (call: any) => /正在压缩|Compact In Progress/.test(call[0]?.body?.title ?? ""),
    )
  ...
  test("clears lock after max recovery attempts exhausted", async () => {
    ...
    const failureToast = toastCalls.find(
      (call: any) => /自动压缩失败|Auto Compact Failed|Auto[- ]?compaction failed|Recovery Failed|Summarization Failed|Compaction Failed/.test(call[0]?.body?.title ?? ""),
    )
