...
<user-request>feature-implementation</user-request>
    ...
    test("should bridge opencode base evidence into notepads when enabled", async () => {
      ...
      const manifestDir = join(testDir, ".opencode", "evidence", sessionId)
    ...
    test("should append base evidence note with timestamp when file already exists", async () => {
      ...
      const notepadDir = join(testDir, ".sisyphus", "notepads", "demo")
    ...
    test("write_json=true skips markdown append + history when no changes detected", async () => {
      ...
      // manifest 1
      const manifestDir1 = join(testDir, ".opencode", "evidence", sessionId1)
      ...
      const manifestDir2 = join(testDir, ".opencode", "evidence", sessionId2)
